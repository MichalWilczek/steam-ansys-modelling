...
remap = WindingRemap(inp_data)
mag = Factory.get_magnetic_map_class(winding_list=remap.map_winding_list())
mat = Factory.get_material_properties_class()
QuenchFront = Factory.get_quench_velocity_class()
...
ans.set_initial_temperature(temperature=inp_data.temperature_init)

# to be defined for initial gaussian distribution
...
ans.set_current(node_number="all", value=inp_data.current_init)
...
res_voltage = inp_data.current_init*coil_resistance
...
quench_state_plot = plots.plot_and_save_quench_state(coil_geometry, quench_fronts, iteration=i, time_step=t)
...
plots.create_gif(plot_array=quench_state_plots, filename='video_quench_state.gif')
plots.create_gif(plot_array=quench_temperature_plots, filename='video_temperature_distribution.gif')
ans.save_analysis()
