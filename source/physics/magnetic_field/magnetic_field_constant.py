...
class MagneticFieldConstant(MagneticField):
    ...
    def assign_magnetic_field_to_geometry(self):
        mag_field_dict = {}
        for i in range(self.input_data.geometry_settings.type_input.n_windings):
            key_name = "winding" + str(i+1)
            mag_field_dict[key_name] = self.input_data.magnetic_field_settings.input.B_constant
