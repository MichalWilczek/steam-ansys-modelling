...
class QuenchDetect(object):
    ...
    def detect_quench(self, input_quench_front_vector, temperature_profile, magnetic_field_map):
        ...
        :param temperature_profile: file with nodal temperature as string
        :param magnetic_field_map: magnetic field winding map as dictionary
        ...
        temperature_profile_sliced = QuenchDetect.slice_temperature_profile(
            input_quench_front_vector=input_quench_front_vector_sorted, temperature_profile=temperature_profile)
        if self.testunit:
            temperature_profile_sliced = QuenchDetect.slice_temperature_profile(
                input_quench_front_vector=input_quench_front_vector_sorted, temperature_profile=temperature_profile)
            temp_profile_sliced_quench_detection = QuenchDetect.find_quenched_nodes_testunit(
                sliced_temperature_profile=temperature_profile_sliced, critical_temperature=4.35)
        else:
            temp_profile_windings_sliced = self.slice_temperature_profile_with_respect_to_winding_numbers(
                temperature_profile_sliced)
            temp_profile_sliced_quench_detection = self.find_quenched_nodes(
                temp_profile_windings_sliced, magnetic_field_map)
        new_quenched_fronts_before_rejection = QuenchDetect.find_new_quench_fronts(
            quenched_nodes=temp_profile_sliced_quench_detection)
        new_quench_fronts_list = QuenchDetect.create_new_quench_fronts_list(
        ...
        new_quench_fronts_sorted = QuenchDetect.define_final_new_quench_fronts(
            fronts_list=new_quench_fronts_list_without_repetitions,
            new_quench_fronts_nodes=new_quenched_fronts_before_rejection)
        quench_fronts_position = QuenchDetect.search_quench_length(
            coil_length=self.coil_length, new_quench_fronts_nodes=new_quench_fronts_sorted)
    ...
    def slice_temperature_profile_with_respect_to_winding_numbers(self, sliced_temp_profile):
        ...
        for slice_pro in sliced_temp_profile:
            if len(slice_pro) != 0:
                ...
                winding_temp_dict = self.geo.retrieve_winding_numbers_and_quenched_nodes(
    ...
    def find_quenched_nodes(self, sliced_temp_wind_profile_list, magnetic_map_dict):
        ...
        for item in sliced_temp_wind_profile_list:
            for key in item:
                ...
                critic_temp = self.mat_props.critical_current_density.critical_temperature(magnetic_field=mag_field)
        ...
        for quenched_nodes_sliced_profiles in quenched_nodes:
            ...
            while x_up_node < len(quenched_nodes_sliced_profiles):
                while x_up_node < len(quenched_nodes_sliced_profiles)-1 and \
                        quenched_nodes_sliced_profiles[x_up_node+1][0] - quenched_nodes_sliced_profiles[x_up_node][0] \
                        == 1:
                        x_up_node += 1
    ...
    def find_repetitive_fronts(duplicate):
        ...
        :param duplicate: list of indices of new quench fronts
    ...
    def define_final_new_quench_fronts(fronts_list, new_quench_fronts_nodes):
        ...
        :param new_quench_fronts_nodes: list of lists in each of which
