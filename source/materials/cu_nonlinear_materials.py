...
class CuNonlinearMaterials(object):
    ...
    def calculate_resistivity(self, magnetic_field, rrr):
        """
        ...
        for i in range(len(self.temperature_profile)):
            ...
            cu_resistivity_array[i, 1] = self.resistivity_nist(
                magnetic_field, temperature=self.temperature_profile[i], rrr=rrr)
        return cu_resistivity_array

    def calculate_thermal_conductivity(self, magnetic_field, rrr):
        """
        ...
        for i in range(len(self.temperature_profile)):
            ...
            cu_thermal_conductivity_array[i, 1] = self.thermal_conductivity_nist(
                magnetic_field=magnetic_field, temperature=self.temperature_profile[i], rrr=rrr)
    ...
    def calculate_cv(self):
        ...
        for i in range(len(self.temperature_profile)):
            ...
            cu_cv_array[i, 1] = self.cv_nist(temperature=self.temperature_profile[i])
        return cu_cv_array

    def calculate_thermal_diffusivity(self, magnetic_field, rrr):
        """
        ...
        cu_thermal_conductivity = self.calculate_thermal_conductivity(magnetic_field=magnetic_field, rrr=rrr)
        cu_cv = self.calculate_cv()
    ...
    @staticmethod
    def resistivity_nist(magnetic_field, temperature, rrr):
        """
    ...
    @staticmethod
    def thermal_conductivity_nist(magnetic_field, temperature, rrr):
        """
        ...
        thermal_conductivity = k_n * \
            (CuNonlinearMaterials.resistivity_nist(magnetic_field=0.0, temperature=temperature, rrr=rrr) /
             CuNonlinearMaterials.resistivity_nist(magnetic_field=magnetic_field, temperature=temperature, rrr=rrr))
