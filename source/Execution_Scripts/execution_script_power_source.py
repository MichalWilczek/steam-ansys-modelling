...
mag = case.get_magnetic_map_class(winding_start=1, winding_end=2, number_of_reels=2)
...
ans.select_nodes_in_analysis(coil_geo, x_down_node=251, x_up_node=251)
...
for qf in quench_front_new:
    quench_fronts.append(QuenchFront(x_down=qf[0], x_up=qf[1], label=quench_label, q_v_interpolation_f=qv_map.f_interpolation))
    quench_label += 1
...
plots.plot_resistive_voltage(voltage=resistive_voltage, time_step=t, iteration=i, additional_descr="ansys")
