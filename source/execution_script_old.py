...
number_windings = 2             # number of windings analyzed
...
if dimensionality == "1D":
    ...
    nodes_to_select = geo_2d.convert_imaginary_nodes_set_into_real_nodes(x_down_node=quench_fronts[0].x_down_node, x_up_node=quench_fronts[0].x_up_node)
...
# to come back here!!!
# quench mapping has a bug..., quench appears at the wrong place
# check the solver cause temperature becomes negative...
...
for i in range(1, 21):
    ...
    for qf in quench_fronts:
        ...
        if dimensionality == "1D":
            ...
            nodes_to_select = geo_2d.convert_imaginary_nodes_set_into_real_nodes(x_down_node=qf.x_down_node, x_up_node=qf.x_up_node)
            nodes_to_select_ansys = geo_2d.prepare_ansys_nodes_selection_list(real_nodes_list=nodes_to_select)
            ans.select_nodes_list(nodes_list=nodes_to_select_ansys)
        ans.select_elem_from_nodes()
        ans.enter_preprocessor()
